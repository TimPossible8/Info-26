...
# =========================================================
# KI-MODELL – lokales Keras/H5-Modell
# =========================================================
MODEL_DATEI = BASE / "model" / "keras_model.h5"
LABEL_DATEI = BASE / "model" / "labels.txt"
...
def load_ai_model():
    ...
    if LABEL_DATEI.exists():
        labels = [
            line.strip()
            for line in LABEL_DATEI.read_text(encoding="utf-8").splitlines()
            if line.strip()
        ]
...
def _find_matching_label(label):
    ...
    for key, value in AI_LABELS["category"].items():
        if normalized == key.lower() or normalized == value.lower():
            return value

    return label.strip() or "Sonstiges"
...
def run_ai_scan(image: Image.Image):
    ...
    try:
        ...
        ergebnis = {
            "kategorie": category,
            "konfidenz": round(best_score, 2),
            "farbe": farbe,
            "farbe_konfidenz": round(farbe_score, 2),
            "stil": stil,
            "tags": [],
            "modell": "keras-h5",
        }
...
def run_ai_scan(image: Image.Image):
    ...
    try:
        ...
        return None, (
            ...
            "und installiere TensorFlow (pip install tensorflow). "
            f"Details: {exc}"
        )
